...
const Cart = () => {
  ...
  // Map of cartItemId -> max stock for that item (color+size)
  ...
  const fetchCart = async (uid) => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE}/api/Cart/user/${uid}`);
  ...
  // After cart items load/update, fetch per-item stock from product details
  useEffect(() => {
    const buildStocks = async () => {
      if (!cartItems || cartItems.length === 0) {
        setItemStocks({});
        return;
      }
      ...
      // Cache product details to avoid refetching the same product multiple times
      const productCache = new Map();
      const newStocks = {};

      // Helper: safely get colorId & sizeId from various API formats
      ...
      for (const item of cartItems) {
        try {
          const productId = item.productId ?? item.product?.productId ?? null;
          if (!productId) {
            // no product id -> assume unlimited? safer to set 0 so "+" disables
            newStocks[item.cartItemId] = 0;
            continue;
          }

          // Use cache or fetch product detail
          let productDetail = productCache.get(productId);
          if (!productDetail) {
            const res = await fetch(`${API_BASE}/api/Product/GetProductById/${productId}`);
            const data = await res.json();
            if (data && data.success && data.product) {
              productDetail = data.product;
              productCache.set(productId, productDetail);
            } else {
              // failed -> no stock info
              newStocks[item.cartItemId] = 0;
              continue;
            }
          }

          // Find stock for the cart item's selected color/size
          const colorId = getColorId(item);
          const sizeId = getSizeId(item);

          let maxStock = 0;

          // First, try direct stock fields on item (if your API already returns them)
          // Supports a variety of possible property names
          const directStock =
            item.availableStock ??
            item.stockAvailable ??
            item.stock ??
          ...
          if (typeof directStock === 'number') {
            maxStock = Math.max(0, directStock);
          } else {
            // Fallback: resolve stock from product detail’s colors and sizes
            const colorObj =
            ...
            if (colorObj && Array.isArray(colorObj.productSizes)) {
              const sizeObj = colorObj.productSizes.find(
                (s) =>
                  s.sizeId === sizeId ||
                  s.SizeId === sizeId
              );
              ...
              if (sizeObj) {
                // Prefer sizeObj.stock, fallback to any possible naming
                const szStock =
                  sizeObj.stock ??
                  sizeObj.Stock ??
                  sizeObj.availableStock ??
                  sizeObj.quantity ??
                  0;
                const isAvailable =
                  sizeObj.isAvailable ??
                ...
                maxStock = isAvailable ? Math.max(0, Number(szStock) || 0) : 0;
              }
            }
          }

          newStocks[item.cartItemId] = maxStock;
        } catch (err) {
          console.error('Error resolving stock for cart item:', err);
          newStocks[item.cartItemId] = 0;
        }
      }

      setItemStocks(newStocks);
  ...
  const updateQuantity = async (cartItemId, nextQuantity) => {
    ...
    const maxStock = itemStocks[cartItemId];
    ...
    try {
      const response = await fetch(`${API_BASE}/api/Cart/item/${cartItemId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const clearCart = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/Cart/user/${userId}`, {
        method: 'DELETE'
      });
  ...
  const getImageUrl = (item) => {
    if (item.colorImageUrl) {
      return `${API_BASE}${item.colorImageUrl}`;
    }
    if (item.color?.imageUrl) {
      return `${API_BASE}${item.color.imageUrl}`;
    }
    if (item.product?.productColors?.[0]?.productImages?.[0]?.imageUrl) {
      return `${API_BASE}${item.product.productColors[0].productImages[0].imageUrl}`;
    }
    return 'https://via.placeholder.com/120x120?text=No+Image';
  };
  ...
  const getPrice = (item) => item.unitPrice || item.product?.price || 0;
          ...
          {cartItems.length === 0 ? (
            ...
            <div className="row">
              <div className="col-lg-8">
                <div className="cart-items">
                  {cartItems.map((item) => {
                    const maxStock = itemStocks[item.cartItemId];
                    const plusDisabled =
                      stocksLoading ||
                    ...
                    return (
                      <div key={item.cartItemId} className="cart-item">
                        ...
                        <div className="cart-item-details">
                          ...
                          <p className="unit-price">₹{getPrice(item).toFixed(2)} each</p>
